  ...
  <div v-loading="loading">
    ...
    >
      ...
      <div class="row mt-1 d-flex flex-row justify-content-end">
        <button class="btn btn-sm btn-primary" @click="changeObservation">Cambiar</button>
      </div>
    </el-dialog>
    ...
    <div id="ordens " class="border-dark rounded-top">
      ...
      <div v-if="ordens.length > 0" class="row d-flex flex-row p-2">
        <div class="col-12 d-flex justify-content-end">
          <el-button-group>
            ...
            <el-button type="danger" @click="cancelGeneralOrden" class="btn btn-sm">
      ...
      <div class="row p-2" v-if="localOrden.length != 0">
        ...
        <div class="col-6 d-flex justify-content-end">
          <button @click="submit" class="btn btn-success btn-sm">Enviar pedido</button>
        </div>
      </div>
          ...
          <div class="col-12">
            <p class="h4 txt-info p-10 txt-primary f-w-700">
              ...
              <a class="badge badge bg-dark text-white" href="javascript:void(0)">
                ...
                <template v-else>{{ localOrden.length }}</template>
              </a>
              Ordenes Pendiente
            </p>
          </div>
      ...
      >
        <div class="row p-r-10">
          <div class="col-12 f-w-700 text-end p-t-5">POR ATENDER S/ {{ totalOrden.toFixed(2) }}</div>

          <div class="col-12 f-w-700 text-end p-t-5">ATENDIDO S/ {{ totalOrdenItems.toFixed(2) }}</div>

          <div class="col-12 f-w-700 text-end p-t-5 p-b-5">TOTAL S/ {{ total.toFixed(2) }}</div>
        </div>
      </div>
    </div>
    <el-dialog
    ...
    >
      <div class="row mt-1">
        <h6
          class="fw-bold"
        >Para poder eliminar la orden debe --------- ingresar un motivo y su PIN de usuario.</h6>
      </div>
      <div class="row mt-1">
        <div class="col-12">
          <el-input
  ...
  methods: {
    ...
    calculateTotal(w = null) {
      ...
      _.forEach(this.ordens, function(values) {
        OrdenPenAtendidos =
          parseFloat(OrdenPenAtendidos) + values.quantity * values.price;
      });
