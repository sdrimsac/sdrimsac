...
use Modules\Restobar\Events\StockEvent;
...
use Modules\Restobar\Events\OrdenPaidEvent;
use Modules\Restobar\Http\Requests\ExpensesRequest;
use Modules\Restobar\Http\Resources\OrdenCollection;
use Modules\Restobar\Http\Resources\ExpensesResource;
use Modules\Restobar\Http\Resources\FoodCollection;
use Modules\Restobar\Models\BoxesDetail;
...
class RestobarController extends Controller
{
    ...
    public function print_box(Request $request)
    {
        ...
        try {
            $pdf = PDF::loadView('restobar::cash.expense_income_ticket', compact(
                'company',
                'box',
                'date',
                'configuration',
    ...
    public function foods(Request $request)
    {
        ...
        $foods = Food::query()->whereHas(
            ...
            function ($query) use ($warehouse_id, $search_by_series, $value, $configuration, $establishment, $brand_id, $customer_unit_type_id, $color_size_enabled) {
                ...
                /* if ($value) {
                    $query->whereHas('item_unit_types', function ($query) use ($value) {
                        $query->where('unique_code', 'LIKE', '%' . $value . '%');
                    });
                } */
            }
        );
    ...
    /* public function pos()
    {
        ...
        if ($cash == null) {
            return redirect()->route('restobar.cash');
        }
    ...
    public function pos()
    {
        ...
        $pending_order = 0;
        return view('restobar::worker.pos', compact('pending_order', 'area_id', 'cash_id', 'worker', 'establishments', 'configuration', 'auth_login', 'company', 'desarrollador'));
    }
    ...
    public function index()
    {
        $establishments = Establishment::where('id', auth()->user()->establishment_id)->first();
        $configuration = Configuration::first();
        return view('restobar::pos', compact('configuration', 'establishments'));
    }
    ...
    public function expenses()
    {
        ...
        if ($cash == null) {
            return redirect()->route('restobar.cash.index');
        }
        ...
        $cashid = $cash->id;
        //dd($categoryid);
        return view('restobar::expenses.index', compact(
            'cashid',
            'groupid',
            'categoryid',
            'userid',
    ...
    public function income()
    {
        ...
        $soaptypeid = $company->soap_type_id;
        //dd($categoryid);
        return view('restobar::income.index', compact('groupid', 'categoryid', 'userid', 'subcategoryid', 'soaptypeid'));
    }
