...
use Modules\Restobar\Events\OrdenCancelEvent;
use Modules\Restobar\Events\OrdenPendingEvent;
use Modules\Restobar\Http\Resources\ObservationCollection;
use Modules\Restobar\Http\Resources\OrdenCollection;
...
use Modules\Restobar\Events\StockRealEvent;
use Modules\Restobar\Models\CustomerAddresses;
use Modules\Restobar\Models\Delivery;
use Modules\Restobar\Models\UserScheduleAppointment;
...
class OrdenController extends Controller
{
    ...
    {
        return view('restobar::configuration.observations');
    }
    ...
    public function printTicket(Request $request)
    {
        ...
        $re_printer = $request->re_printer ?? false;
        $to_kitchen = false;
        if ($precuenta) {
            $to_kitchen = true;
        }
        ...
        $view = $configuration->printing_comanda ? 'restobar::ordens.ticketnew' : 'restobar::ordens.ticket';
    ...
    {
        $configuration = Configuration::first();
        $is_restaurant = $configuration->restobar_home;
        $show_unit_ticket = $configuration->show_unit_types_ticket;
        $company = Company::first();
        $orden = $request->id;
        $ordens_items_extern = explode("_", $request->ids);
    ...
    public function index()
    {
        ...
        return view('restobar::ordens.index', compact('configuration'));
    }
    ...
    public function recordWorker($id, Request $request)
    {
        ...
        $re_printer = filter_var($request->get('re_printer', false), FILTER_VALIDATE_BOOLEAN);
        ...
        if ($to_kitchen) {
            // Si va a cocina, usa área de cocina
            $area = Area::where('description', 'like', '%COCIN%')->first();
            $area_id = $area->id ?? null;
        } else {
            // Siempre usa el área de caja por defecto (NO la del usuario/mozo)
            $area_id = Area::getCajaAreaIdByTableId($orden->table_id);
            $area = Area::find($area_id);
        }
        ...
        if ($printer == null) {
            $area_caja_id = Area::getCajaAreaIdByTableId($orden->table_id);
            $area_caja = Area::find($area_caja_id);

            if ($area_caja && $area_caja->printer) {
                $printer = $area_caja->printer;
        ...
        if ($orden == null) {
            ...
            return [
                ...
                'print'   => url('') . "/restobar/worker/print-ticket?id={$id}&ids={$ids_string}&area_id={$area_id}&precuenta={$precuenta}&mozo_name={$mozo_name}&zone_name={$zone_name}"
            ];
        }
    }

    public function recordWorkerRePrint($id, Request $request)
    {
        $orden = Orden::find($id);
        if (!$orden) {
            return [
        ...
        if ($orden_items->isEmpty()) {
            return [
                'success' => false,
                'print'  => "No se encontraron items en la orden."
            ];
        }
        ...
        foreach ($items_by_area as $area_id => $items) {
            $area = Area::find($area_id);
            if (!$area || !$area->printer) {
                return [
                    'success' => false,
                    'print'  => "No se encontró una impresora configurada para el área con ID {$area_id}."
                ];
            }
            ...
            // Enviar a imprimir
            //event(new PrintEvent($orden->id, "00", true, $area_id, $orden_item_ids, false, $precuenta));
            event(new PrintEvent($orden->id, "00", true, $area_id, $orden_item_ids, false, $precuenta, null, $re_printer));
            // Agregar resultado a lista
            $impresiones[] = [
                'printer' => $area->printer,
                'area' => $area->description,
                'print_url' => url('') . "/restobar/worker/print-ticket?id={$orden->id}&ids={$ids_string}&area_id={$area_id}&precuenta={$precuenta}&mozo_name={$mozo_name}&zone_name={$zone_name}"
            ];
        }
        ...
        if ($configuration->image_comand) {
            ...
            foreach ($items_by_area as $area_id => $items) {
                ...
                $print_data[] = [
                    'area_id' => $area_id,
                    'printer' => $printer,
                    'print_url' => url('') . "/restobar/worker/print-ticket?id={$id}&ids={$ids_string}&area_id={$area_id}" .
    ...
    public function DeliveryOrden(Request $request)
    {
        ...
        try {
            $pdf = PDF::loadView('restobar::ordens.delivery', compact(
                'configuration',
                'establishment',
                'company',
                'orden',
    ...
    public function DeliveryPrinter(Request $request)
    {
        ...
        if ($configuration->restobar_home) {
            ...
            return [
                ...
                'direct_printing' => (bool) $establishment->direct_printing,
                'printer_serve' => $establishment->printer_serve,
                'print'   => url('') . "/restobar/delivery/ticket?id={$id}&establishment_id={$establishment->id}"
            ];
        }
    }
