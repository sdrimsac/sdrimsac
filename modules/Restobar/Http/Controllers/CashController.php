...
use Modules\Restobar\Events\GenerateExcelReportJob;
use Modules\Restobar\Events\PrintEvent;
use Modules\Restobar\Http\Requests\ExpensesRequest;
use Modules\Restobar\Http\Resources\CashIncomePrincipalCollection;
...
use Modules\Restobar\Models\CashOpeningBalance;
use Modules\Restaurant\Models\CashOpeningBalances as ModelsCashOpeningBalances;
use Modules\Restobar\Models\CashOrderSession;
...
class CashController extends Controller
{
            ...
            if ($last_principal_cash) {
                ...
                foreach ($balances as $balance) {
                    ...
                    Box::create([
                        ...
                        'incomes' => true,
                        'date' => date('Y-m-d'),
                        'method' => $balance->method ?? 'Efectivo',
                        'group_id' => 1,
                        'subcategory_id' => 1,
                        'category_id' => 1,
                        'soap_type_id' => '01',
                        'description' => 'Saldo de la caja anterior por método ' . ($balance->method ?? 'Efectivo'),
                        'state' => 1,
                        'user_id' => auth()->user()->id,
                    ]);
                }
    ...
    public function boxClose(ExpensesRequest $request)
    {
        $configuration = Configuration::first();
        $cash_id = $request->cashid;
    ...
    /* public function records_principal(Request $request)
    {
        ...
        // 👇 agrupamos por cash_id
        $records = $records
            ->selectRaw('cash_id, SUM(amount) as total_amount, MIN(created_at) as created_at')
            ->groupBy('cash_id')
