...
use Modules\Restobar\Events\OrdenReadyEvent;
use Carbon\Carbon;
use Exception;
...
use Modules\Restobar\Events\OrdenReadyListEvent;
...
use Modules\Restobar\Models\OrdenDelete;
        ...
        if ($configuration->image_comand) {
            ...
            try {
                ...
                $print_data = [
                    'area_id' => $area_id,
                    'printer' => $printer,
                    'print_url' => url('') . "/restobar/worker/print-ticket?id={$orden->id}&ids={$orden_item->id}&area_id={$area_id}&precuenta=false&cancel_orden_item={$cancel_orden_item}",
                ];
