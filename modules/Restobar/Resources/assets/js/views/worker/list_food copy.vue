...
export default {
    ...
    created() {
        ...
        //this.$root.$on("addDataMozo", this.handleAddEvent);
    ...
    methods: {
        ...
        /* handleReloadData(newProduct) {
            // Find the new product in the list
            const index = this.listFoods.findIndex(
                food => food.id === newProduct.id
            );
            if (index !== -1) {
                this.addFood(index);
            }
        ...
        handleReloadData(data) {
            if (!data || !data.id) {
                console.error("Datos inválidos recibidos en reloadData:", data);
                return;
            }

            let index = this.foodList.findIndex(item => item.id === data.id);

            if (index !== -1) {
                this.foodList[index].name = data.name;
            } else {
                console.warn("Producto no encontrado:", data);
            }
        },
        handleAddEvent(newProduct) {
            console.log('Received in add event:', newProduct); // Log to check if the event is received
            // Find the new product in the list
            const index = this.listFoods.findIndex(food => food.id === newProduct.id);
            if (index !== -1) {
                this.addFood(index);
            }
        }
