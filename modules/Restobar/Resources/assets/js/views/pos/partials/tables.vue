                ...
                <div class="" v-if="ordens.length == 0 || hasSelectedOrdenToChange">
                    ...
                    <div class="d-flex justify-content-center p-2" style="background: #616161;">
                        ...
                        <button
                            ...
                            :class="`btn ${zone_id == zone.id ? 'btn_whatsappsmall' : 'btn_guardarsmall' }`"
                    ...
                    <div class="d-flex flex-wrap justify-content-left">
                        <div class="col-3" v-for="(ord, idx) in ordens" :key="idx">
                            <button @click="sendOrdens(ord)" type="button" class="btn btn-primary p-1 m-1 ">
                                ...
                                <span class="h4 text-white">{{ ord.ref ? ord.ref : "Sin referencia" }}</span>
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    data() {
        return {
            ...
            ordens: [],
            loading: false,
            resource: "/restobar/tables/tables",
            tables: [],
            all_tables: [],
            zones: [],
            showOrdens: false,
    ...
    methods: {
        ...
        async DeliveryTicket(ord) {
            try {
                const response = await this.$http.get(
                    `/restobar/delivery/DeliveryPrinter?id=${ord.id}`
                );
        ...
        async selectTable(table) {
            ...
            try {
                const response = await this.$http(
                    `/restobar/tables/orden/${table.id}`
                );
                if (response.status == 200) {
        ...
        async getTablesDelivery() {
            try {
                this.loading = true;
                const response = await this.$http(
                    "/restobar/tables/getTablesDelivery"
                );
                console.log("Respuesta del servidor:", response.data);
