...
{ 
    public function getPaymentDestinations(){
        ...
        if (auth()->user()->type === 'logistica' || 
            (auth()->user()->type === 'admin' && $this->isUserLogistica(auth()->user()->worker_type_id))) {
    ...
    private function isUserLogistica($worker_type_id) {
        return DB::table('workers_type')->where('id', $worker_type_id)->value('description') === 'LOGISTICA';
    }
