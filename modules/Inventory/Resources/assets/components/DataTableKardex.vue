...
export default {
  ...
  mounted() {
    ...
    // Si configuration.consolidated_quotations es true, max_quantity debe ser true por defecto
    if (this.configurations.consolidated_quotations == true) {
      this.max_quantity = true;
    }
    // Llama a la función parsedMaxQuantity una vez para aplicar el formato por defecto
      ...
      this.records = records.map(record => {
        if (this.max_quantity) {
          record.input =
            record.input == "-"
              ? "-"
              : this.maxQuantityFormat(record.input, item.max_quantity, item);
          record.output =
            record.output == "-"
              ? "-"
              : this.maxQuantityFormat(record.output, item.max_quantity, item);
          record.balance =
            record.balance == "-"
              ? "-"
              : this.maxQuantityFormat(record.balance, item.max_quantity, item);
        } else {
          record.input =
            record.input == "-"
              ? "-"
