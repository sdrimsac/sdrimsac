...
export default {
    ...
    methods: {
        ...
        async ordenNew(id) {
            try {
                ...
                if (response.status === 200 && response.data) {
                    ...
                    this.selectOrden(orden.id, orden.orden_items);
                    return orden;
                }
            } catch (error) {
        ...
        selectOrden(id) {
            console.log("ver que llega:", id);
            if (this.changingOrden) {
                this.$emit("changeOrdenEvent", orden.id);
                console.log("ver que llega evento emitido:", orden.id);
                return;
            }
            this.ordenTitle = `Orden #${orden.id}`;
