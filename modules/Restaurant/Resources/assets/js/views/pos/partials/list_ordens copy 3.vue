            ...
            <div class="d-md-flex flex-wrap justify-content-between ">
                ...
                <div class="pos-action-buttons">
                    ...
                    <button
                        ...
                        class="btn btn-primary pos-btn mb-2"
                        type="button"
                        @click="trigerFunction(5)"
                        style="padding: 5px 5px;"
                    >
                        <i class="fas fa-tasks" style="margin-right: 2px;"></i
                        >Ver
                        <br />
                        Ordenes
                    </button>
                    ...
                    <button
                        v-if="configuration.credit_list && !isSeller"
                        class="btn btn-primary pos-btn mb-2"
                        type="button"
                        @click="trigerFunction(6)"
                        style="padding: 5px 5px;"
                    >
                        <i
                            ...
                            <div class="row">
                                <div class="col-md-6 col-12">
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                style="color: darkblue; font-weight: bold; font-size: 12px;"
                                            >
                                                Productos:
                                            </span>
                                            <br />
                                            ...
                                            >
                                            <el-tooltip
                                                content="Total de producto a vender"
                                                placement="top"
                                                effect="light"
                                            >
                                            </el-tooltip>
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        <strong
                                                                            style="font-weight: bold; font-family: 'Arial Black', Arial, sans-serif; font-size: 1.2rem;"
                                                                        >
                                                                            {{ (order_pend._total_line !== undefined ? order_pend._total_line : (order_pend.price * order_pend.quantity)).toFixed(2) }}
                                                                        </strong>
                                                                    </el-tag>
                                                                    <el-input
                                                                        v-else
...
export default {
    ...
    methods: {
        ...
        // order: objeto del arreglo localOrden u ordens
        // return { base, igv, total, discount_amount }
        _calcItemAmounts(order){
            try {
                ...
                } else { // Exonerado / Inafecto: todo es base, sin IGV
        ...
        // Genera estructura discounts tipo "00" (afecta base) por cada item con descuento > 0
        _attachItemDiscounts(items){
            try{
                items.forEach(item => {
                    // Estructura esperada del item según el uso actual
                    // Se asume item.food.item.discount (monto o porcentaje) y item.discount (flag porcentaje)
                    if(!item || !item.food || !item.food.item) return;
                    const qty = Number(item.quantity)||0;
                    const unitPrice = Number(item.price)||0;
                    if(qty <= 0 || unitPrice < 0) return;
                    const lineTotal = qty * unitPrice; // total original (con IGV si gravado)
                    ...
                    if(discountInput <= 0) return; // sin descuento
                    let discountAmount = 0; // monto de descuento ingresado/calculado sobre el total de la línea (incluye IGV si aplica)
                    if(isPercent){
                        discountAmount = lineTotal * (discountInput/100);
                    } else {
                    ...
                    // Base original antes del descuento (sin IGV si aplica)
                    let baseOriginal = 0;
                    if(igvType == '10'){
                        baseOriginal = lineTotal / (1 + 0.18);
                    } else {
                        baseOriginal = lineTotal; // exonerado/inafecto
                    }
                    if(baseOriginal <= 0) return;
                    // Para items gravados, el "amount" del descuento tipo 00 debe ir SIN IGV
                    const netDiscount = igvType == '10'
                        ? (discountAmount / (1 + 0.18))
                        : discountAmount;
                    const factor = _.round(netDiscount / baseOriginal, 4);
                    const amount = _.round(netDiscount, 2);
                    const base = _.round(baseOriginal, 2);
                    // Asignar discounts (sobrescribe si ya existía)
                    item.discounts = [
        ...
        calculateTotal(w = null) {
            ...
            this.total = 0.0;
            this.totalOrden = 0.0;
            let totalGravado = 0, totalExonerado = 0, totalIgv = 0, totalDescuentos = 0;
