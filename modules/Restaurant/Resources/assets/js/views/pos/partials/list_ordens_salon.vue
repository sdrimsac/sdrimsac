<template>
    <div v-loading="ordenLoading">
        <div>
            ...
            <!-- <div v-if="currentAppointment" class="appointment-info mb-4">
                <div class="card">
                    <div class="card-header bg-info text-white">
                        <h5 class="mb-0">Cita en Atención</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <p><strong>Cliente:</strong> {{ currentAppointment.client.name }}</p>
                                <p><strong>Servicio:</strong> {{ currentAppointment.service.description }}</p>
                            </div>
                            <div class="col-md-4">
                                <p><strong>Fecha:</strong> {{ formatDate(currentAppointment.appointment_date) }}</p>
                                <p><strong>Hora:</strong> {{ currentAppointment.start_time }} - {{ currentAppointment.end_time }}</p>
                            </div>
                            <div class="col-md-4">
                                <p><strong>Estilista:</strong> {{ currentAppointment.user.name }}</p>
            ...
            >
                ...
                <div class="row col-12" v-if="currentAppointment">
                    <div class="col-6">    
                        <h3 class="text-white"> 
                            <strong style="padding-left: 20px">
                                Cita: {{ currentAppointment.client.name }}
                            </strong>
                        </h3>
                    </div>
                </div>
                ...
                <div class="row col-12">
                    <!-- <div class="h5 text-white col-6" style="padding-left: 25px">
                        <strong >
                            <template>
                                {{
                                    currentAppointment && currentAppointment.orden_id ? "CITA" : "VENTA DIRECTA"
                                }}
                            </template>
        ...
        <!-- Botón de enviar (modificar la condición existente) -->
        <div class="fixed-bottom bg-white border-top p-3" v-if="rows.length > 0 && currentAppointment">
    ...
    methods: {
        ...
        async sendOrden() {
            ...
            try {
                const responses = await this.$http.post("/caja/worker/send-orden", {
                    id: this.currentAppointment.orden_id,
                    ref: "Cita de " + this.currentAppointment.client.name,
                    items: this.localOrden,
        ...
        async payOrden(offert = null) {
            ...
            let form_submit = {
                ...
                to_carry: this.to_carry,
                clientSaleNoteNumber: this.currentAppointment ? this.currentAppointment.client.number : null,
                orden: {
                    table_id: 1,
                    status_orden_id: 1
                },
        ...
        async sendProducts() {
            if (!this.currentAppointment || this.rows.length === 0) return;

            this.loading = true;
            try {
                const response = await this.$http.post(`/caja/estilista/appointments/${this.currentAppointment.id}/products`, {
                ...
                if (response.data.success) {
                    this.$message.success('Productos guardados correctamente');
                    this.clearProducts();
                    this.$emit('products-saved', this.currentAppointment.id);
                } else {
                    this.$message.error(response.data.message || 'Error al guardar productos');
                }
            } catch (error) {
                console.error('Error al guardar productos:', error);
                this.$message.error('Error al guardar productos');
            } finally {
                this.loading = false;
            }
        },
        ...
        clearProducts() {
            this.rows = [];
            this.$emit('clear-current-appointment');
        },
