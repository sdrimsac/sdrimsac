    ...
    <el-dialog @open="open" @close="close" append-to-body :show-close="true" :visible="showDialog" :title="null"
        ...
        <template #title>
            <div style="display: grid; grid-template-columns: 1fr auto; gap: 300px; align-items: center;">
                ...
                <span style="font-weight: bold; font-size: 16px;">Listado de Productos</span>

                <!-- Título de la derecha con el valor de form.total, añadiendo el simbolo 'S/' -->
                <span style="font-size: px; color: #fff; text-align: right;">
                   Total Venta :  S/ {{ form.total }}
                </span>
            </div>
        </template>
        <div class="row">
            <table class="table table-responsive" >
                ...
                <tbody>
                    <tr v-for="(item, index) in items" :key="index" :style="{
                        ...
                        <td style="padding: 12px 15px; border-bottom: 1px solid #ddd;">
                            ...
                            <div v-if="item.affectation_igv_type_id == 15 || item.affectation_igv_type_id == 21">
                                <span class="badge badge-success"
                                    style="background-color: #1E88E5; color: white; padding: 4px 8px; font-size: 0.9rem; border-radius: 12px;">
                                    Producto Gratuito
                                </span>
                            </div>
                        </td>
                        ...
                        <td style="padding: 12px 15px; border-bottom: 1px solid #ddd;">{{ formatNumber(item.total) }}
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <span slot="footer" class="dialog-footer" >
            <div class="form-actions d-flex justify-content-end ">
                <el-button class="btn_cancelarsmall" icon="fas fa-times" @click.prevent="close()">
                    <span>Cancelar</span>
                </el-button>
            </div>
        </span>
