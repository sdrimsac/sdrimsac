                  ...
                  <div class="col-2 d-flex flex-column">
                    ...
                    <el-checkbox
                      ...
                      @change="
                                            ...
                                            "
                    >
                      <h2 class="font-weight-bold custom-text-size">Buscar por series</h2>
                    </el-checkbox>
                    <el-checkbox
                    ...
                    >
                      <h2 class="font-weight-bold custom-text-size">Barcode</h2>
                    </el-checkbox>
                    <!-- <el-checkbox
                                            v-model="type_code"
                      ...
                      <div class="col-12">
                        <el-checkbox
                          ...
                          @change="
                                                    ...
                                                    "
                        >
                          <h2 class="text-muted text-small">Barcode</h2>
                        </el-checkbox>
                      </div>
                    </div>
...
export default {
  ...
  computed: {
    ...
    canAddItem() {
      if (this.isSeller && this.configuration.consolidated_quotations) {
        return !(
          this.customer_unit_type_id == null || this.customer_unit_type_id == ""
        );
      }
      return true;
    }
  ...
  methods: {
      ...
      this.tablesClean.forEach(t => {
        if (t.is_cleaning) {
          ...
          if (diff <= 0) {
            ...
            diff = Math.floor(diff / 60);
            let minutes = diff % 60;
            t.time_to_finish = `${minutes < 10 ? "0" : ""}${minutes}:${
              seconds < 10 ? "0" : ""
            }${seconds}`;
          }
        }
    ...
    formatedStockPresentation(
      { max_quantity, item_unit_types, max_quantity_description, unit_type },
      stock
    ) {
        ...
        ...orden_items.map(o => {
          ...
          if (orden.unit_type_id) {
            ...
            if (unit_type) {
              ...
              orden.type_quantity = Number(unit_type.quantity_unit);
            }
          }
          return {
            ...orden,
    ...
    forma(i) {
      return {
        ...
        total_base_igv:
          i.sale_affectation_igv_type_id == 10
            ? (i.sale_unit_price * i.quantity) / (1 + this.percentage_igv / 100)
        ...
        if (lots_group.length != 0) {
          for (let i = 0; i < lots_group.length; i++) {
            ...
            if (lot.quantity > 0 && !added) {
              ...
              orden.lotes = [...orden.lotes, { ...lot, quantitySelected: 0 }];
            }
          }
        }
        ...
        if (type != null) {
          ...
          if (indexFind != -1) {
            if (selectSerie) {
              ...
              if (serieFind) {
                let exist = this.localOrden[indexFind].series.find(
                  s => s.id == serieFind.id
                );
                if (!exist) {
      ...
      try {
        if (status == 0) {
          ...
          if (res) {
            ...
            const response = await this.$http.get(`/caja/worker/ordens-status`);
            if (response.status == 200) {
          ...
          if (status == 3) {
            for (let index = 0; index < this.selectedCatIds.length; index++) {
              const response = await this.$http.get(
                `/caja/worker/ordens-ready/` + this.selectedCatIds[index]
              );
      ...
      for (let cat = 0; cat < this.tableSelect.ordens.length; cat++) {
        for (
          ...
          for (let index = 0; index < this.selectedCatIds.length; index++) {
            if (this.selectedCatIds[index] == idOrden) {
              this.addNewFood(
                this.tableSelect.ordens[cat].orden_items[indexxx].food_id
              );
            }
          }
        }
    ...
    calculateItem(index, quantity = 0, price = 0) {
      ...
      if (this.form.items[index].affectation_igv_type_id == "10") {
        ...
        this.form.items[index].total_base_igv = _.round(
          this.form.items[index].total / (1 + this.percentage_igv / 100),
          2
        );
    ...
    calculateTotalVariation(formVariation) {
      ...
      formVariation.items.forEach(row => {
        ...
        if (row.sale_affectation_igv_type_id === "10") {
          total_igv += _.round(
            parseFloat(row.total_value) * (this.percentage_igv / 100),
            2
          );
    ...
    async getTablesToLeave() {
      try {
        const response = await this.$http.get(`/caja/rooms/tables_to_leave`);
        if (response.status == 200) {
    ...
    async getTablesToClean() {
      try {
        const response = await this.$http.get(`/caja/rooms/tables_to_clean`);
        if (response.status == 200) {
          const { data } = response.data;
    ...
    async getTables() {
      ...
      await this.$http.get(`/${this.resource}/tables`).then(response => {
        ...
        this.customer_default =
          response.data.customers_default[0] || response.data.customers[0];
        ...
        this.all_customers = [
          ...this.all_customers.filter(c => c.id != this.customer_default.id),
          this.customer_default
        ];
    ...
    addFood(index = 0, type = null, categoria = null) {
      /* ; */
      if (!this.canAddItem) {
        this.$showSAlert("Error", "Debe seleccionar un cliente", "error");
  ...
  mounted() {
        ...
        if (this.configuration.all_items_pos) {
          for (let index = 0; index < e.data.order_item.length; index++) {
            ...
            if (xFind) {
              ...
              if (index_find !== -1) {
                let nSaldo =
                  parseInt(this.listFoods[index_find].item.stock) -
    ...
    Echo.channel("print_orden").listen(
      ...
      async e => {
        ...
        let sameAreas = this.configuration.print_direct_just_different_areas;
