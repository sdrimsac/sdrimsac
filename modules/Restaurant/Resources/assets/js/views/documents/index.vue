    ...
    <div>
        <div class="container-fluid p-l-0 p-r-0">
                ...
                <div class="col-sm-6">
                  ...
                  <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/dashboard">Dashboard</a></li>
                     <li class="breadcrumb-item active"><span class="text-muted">Facturas - Boletas</span></li>
                  </ol>
                </div>
              <div class="col-12 col-md-6 d-flex align-items-start justify-content-end">
                <!-- Contact Button Start -->
                <button type="button" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto" v-if="typeUser=='admin' || typeUser=='superadmin'" @click.prevent="clickNuevo()">
                ...
                <div class="ms-1">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    ...
                    <button class="dropdown-item" type="button" @click.prevent="clickImportSecond()" v-if="import_documents_second == true">Importar Formato 2 </button>
                  </div>
                </div>
                <!-- Dropdown Button End -->
              </div>
          ...
          <div class="container-fluid p-l-0 p-r-0">
            ...
            <h6 class="my-0  text-white">Listado de Comprobante de Pagos</h6>
        </div>
            ...
            <div class="card-body" >
                <data-table :resource="resource">
                    <tr slot="heading">
                      <th class="text-left" v-if="typeUser != 'integrator'">Acciones</th>
                        <th>#</th>
                        <th>SOAP</th>
                        ...
                        <th class="text-left" v-if="columns.total_exportation.visible">T.Exportación</th>
                        <th class="text-left" v-if="columns.total_free.visible">T.Gratuita</th>
                        <th class="text-left" v-if="columns.total_unaffected.visible">T.Inafecta</th>
                        <th class="text-left" v-if="columns.total_exonerated.visible">T.Exonerado</th>
                    ...
                    <tr slot-scope="{ index, row }" :class="{'text-danger': (row.state_type_id === '11'),
                           ...
                           'border border-primary': (row.state_type_id === '05'),
                           'border border-info': (row.state_type_id === '07'),
                           'border border-dark': (row.state_type_id === '09'),
                           'border border-danger': (row.state_type_id === '11'),
                           'border border-warnnig': (row.state_type_id === '13'),
                           }">
                         <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left" v-if="typeUser != 'integrator'">
                                ...
                                <div class="dropdown-menu dropdown-menu-end" style="">
                                     <a type="button" class="dropdown-item"
                                         ...
                                         <a type="button" class="dropdown-item"
                                                ...
                                                v-if="row.document_type_id != '03'">Volver a recrear
                                        </a>
                                         <a class="dropdown-item"
                                                @click.prevent="clickVoided(row.id)"
                                                v-if="row.btn_voided">Anular</a>
                                         <a type="button" :href="`/${resource}/note/${row.id}`" class="dropdown-item"
                                        v-if="row.btn_note">Nota de Credito</a>

                                         <a type="button" class="dropdown-item"
                                                @click.prevent="clickEdit(row.id)"
                                                v-if="row.state_type_id === '01' || row.state_type_id === '14'">Modificar
                                        </a>
                                         <a type="button" class="dropdown-item"
                                                @click.prevent="clickValidarCpe(row.id)"
                                            >Validar CPE
                                        </a>
                                         <a type="button" class="dropdown-item"
                                                @click.prevent="clickResend(row.id)"
                                                v-if="row.btn_resend && !isClient && row.document_type_id != '03' ">
                                            Reenviar
                                        </a>
                                         <a type="button" class="dropdown-item"
                                         ...
                                         <a type="button" class="dropdown-item"
                                                @click.prevent="clickOptions(row.id,false)">
                                                Imprimir
                                        </a>

                                         <a type="button" class="dropdown-item"
                                    @click.prevent="clickPayment(row.id)">Pagos</a>
                                </div>
                            </div>
                        </td>
                        <td  :class="{'text-dark': (row.state_type_id === '11')}">{{ index }}</td>
                        <td  :class="{'text-dark': (row.state_type_id === '11')}">{{ row.soap_type_description }}</td>
                        <td  :class="{'text-dark': (row.state_type_id === '11')}" v-if="columns.user_name.visible">
                            {{row.user_name}}
                            <br/><small v-text="row.user_email"></small>
                        </td>
                        <td class="text-center" :class="{'text-dark': (row.state_type_id === '11')}">{{ row.date_of_issue }}</td>
                        <td class="text-center" v-if="columns.date_of_due.visible" :class="{'text-dark': (row.state_type_id === '11')}">{{ row.date_of_due }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}">{{ row.customer_name }}<br/><small class="badge bg-dark" v-text="row.customer_number"></small></td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}">{{ row.number }}<br/>
                            <small class="badge bg-dark" v-text="row.document_type_description"></small><br/>
                        ...
                        <td >
                              <el-tooltip v-if="tooltip(row, false)" class="item" effect="dark" placement="bottom">
                                ...
                                <span class="badge" :class="{'bg-danger': (row.state_type_id === '11'), 'badge bg-warning': (row.state_type_id === '13'), 'badge bg-dark': (row.state_type_id === '01'), 'bg-warning': (row.state_type_id === '03'), 'bg-success': (row.state_type_id === '05'), 'bg-info': (row.state_type_id === '07'), 'bg-dark': (row.state_type_id === '09')}">
                                    {{row.state_type_description}}
                                </span>
                            </el-tooltip>
                            <span v-else class="badge" :class="{'bg-danger': (row.state_type_id === '11'), 'badge bg-warning': (row.state_type_id === '13'), 'badge bg-dark': (row.state_type_id === '01'), 'bg-warning': (row.state_type_id === '03'), 'bg-success': (row.state_type_id === '05'), 'bg-info': (row.state_type_id === '07'), 'bg-dark': (row.state_type_id === '09')}">
                                {{row.state_type_description}}
                            </span>
                        ...
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-center">{{ row.currency_type_id }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left" v-if="columns.total_exportation.visible">{{ row.total_exportation }}</td>

                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left" v-if="columns.total_free.visible">{{ row.total_free }}</td>

                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left" v-if="columns.total_unaffected.visible">{{ row.total_unaffected }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left" v-if="columns.total_exonerated.visible">{{ row.total_exonerated }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left">{{ row.total_taxed }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left">{{ row.total_igv }}</td>
                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left">{{ row.total }}</td>
                        <template v-if="row.balance>0">
                            <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left text-danger font-weight-bold">{{ row.balance }}</td>
                        </template>
                        <template v-else>
                            <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-left">{{ row.balance }}</td>
                        </template>
                        <template v-if="row.balance==0.00">
                            <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-center">
                                <span class="badge rounded-pill bg-success text-white">Pagado</span>
                            </td>
                        </template>
                         <template v-else>
                            <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-center">
                                <span class="badge rounded-pill bg-warning text-white">Pendiente</span>
                            </td>
                        </template>

                        <td :class="{'text-dark': (row.state_type_id === '11')}" class="text-center">
                            <button type="button" style="min-width: 41px" class="btn btn-sm btn-primary hover-outline"
...
<script>
    ...
    export default {
        mixins: [deletable],
        props: ['isClient','typeUser','import_documents','import_documents_second','configuration'],
        ...
        methods: {
            ...
            clickValidarCpe(document_id){
                this.loading_data=false
                this.$http.get(`/${this.resource}/validate/${document_id}`)
                    .then(response => {
                        if (response.data.success) {
                            this.$toast.success(response.data.message)
                            this.$eventHub.$emit('reloadData')
