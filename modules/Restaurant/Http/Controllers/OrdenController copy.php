...
class OrdenController extends Controller
{
    ...
    public function printTicket(Request $request)
    {
        ...
        if ($ordenes == null) {
            return [
                "success" => false,
                "message" => "Nº Pedido no existe... qqqqqqqq"
            ];
        }
 ...
 */
    ...
    public function cancelOrden(Request $request) 
    {
        ...
        // After printing, proceed with cancellation
        foreach ($items as $item) {
            $items_message[] = $item->info_item();
            event(new OrdenCancelEvent($item->id));
            $item->delete();
        ...
        $table_id = $orden->table_id;
        if ($table_id) {
            $remaining_orders = Orden::where('status_orden_id', 1)
        ...
        // Delete the order after everything else is done
        $orden->delete();
