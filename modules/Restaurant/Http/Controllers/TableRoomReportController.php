...
use Modules\Restaurant\Exports\HotelRentReportExport;
use Modules\Restaurant\Http\Resources\HotelRentCustomerReportCollection;
use Modules\Restaurant\Http\Resources\HotelRentRoomReportCollection;

class TableRoomReportController extends Controller
{
    public function reportsRooms(Request $request)
    {
    }
    ...
    public function index()
    {
        ...
        return view('restaurant::report.rooms', compact('configurations'));
    }
    ...
    //     $rents = null;
    //     if ($filter_type == '1') {
    //         $rents = HotelRent::select(
    //             'hotel_rents.customer_id',
    //             'hotel_rents.customer',
    //             DB::raw('SUM(hotel_rent_items.duration) as total_duration'),
    //             DB::raw('COUNT(hotel_rent_items.id) as item_count') // Agregamos el conteo
    //         )
    //         ->leftJoin('hotel_rent_items', 'hotel_rents.id', '=', 'hotel_rent_items.hotel_rent_id')
    //         ->groupBy('hotel_rents.customer_id', 'hotel_rents.customer')
    //         ->orderBy('total_duration', 'desc');


    //         return new HotelRentCustomerReportCollection($rents->paginate(config('tenant.items_per_page')));
    //     } else {
    //         $rents = HotelRentItem::select(
    //             'hotel_rent_items.table_id as rent_table_id',
    //             'tables.id as table_id',
    //             'tables.number as table_name',
    //             'floors.name as floor_name',
    //             'towers.name as tower_name',
    //             DB::raw('SUM(hotel_rent_items.duration) as total_duration'),
    //             DB::raw('COUNT(hotel_rent_items.id) as item_count') // Agregamos el conteo
    //         )
    //         ->leftJoin('tables', 'hotel_rent_items.table_id', '=', 'tables.id')
    //         ->leftJoin('floors', 'tables.floor_id', '=', 'floors.id')
    //         ->leftJoin('towers', 'floors.tower_id', '=', 'towers.id')
    //         ->groupBy('hotel_rent_items.table_id', 'tables.id', 'tables.number', 'floors.name', 'towers.name')
    //         ->orderBy('total_duration', 'desc');


    //         return new HotelRentRoomReportCollection($rents->paginate(config('tenant.items_per_page')));
    ...
    public function export(Request $request)
    {
        ...
        if ($filter_type == '1') {
            $rents = HotelRent::select(
                ...
                ->where(function ($query) use ($date_start, $date_end) {
                    if (!is_null($date_start)) {
                        $query->where(function ($subquery) use ($date_start, $date_end) {
                            $subquery->where('hotel_rent_items.checkin_date', '>=', $date_start);
                            // ->orWhereNull('hotel_rent_items.checkin_date');
                        });
                    }
                    if (!is_null($date_end)) {
                        $query->where(function ($subquery) use ($date_start, $date_end) {
                            $subquery->where('hotel_rent_items.checkout_date', '<=', $date_end)
                                ->orWhereNull('hotel_rent_items.checkout_date');
                        });
                    }
                })
                ->groupBy('hotel_rents.customer_id', 'hotel_rents.customer')
            ...
            $rents = HotelRentItem::select(
                ...
                ->groupBy('hotel_rent_items.table_id')
                ->orderBy('total_duration', 'desc')->get();
        }
        if ($type == 'excel') {
            return (new HotelRentReportExport)
                ->records($rents)
                ->date_start($date_start)
                ->date_end($date_end)
                ->filter_type($filter_type)
                ->download('ReporteHabitaciones.xlsx');
        } else {
            ...
            $filename = 'ReporteHabitaciones' . '.pdf';
