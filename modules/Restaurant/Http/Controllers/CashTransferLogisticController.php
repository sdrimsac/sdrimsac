...
class CashTransferLogisticController extends Controller
{
    public function index()
    {
        $configuration = Configuration::first();
        return view('restaurant::cash_transfer_logistic.index', compact('configuration'));
    }
    public function index_report()
    {
        $configuration = Configuration::first();
        return view('restaurant::cash_transfer_logistic.index_report', compact('configuration'));
    }
    ...
    public function availableCreditLogistic()
    {
        $user = auth()->user();
        // Verificar si el usuario tiene un worker_type LOGISTICA
        if (!$user || !$user->worker_type_id) {
            return 0;
        }
        ...
        $cash_principal = Cash::where('user_id', $user->id)
            ->where('state', 1)
            ->first();
        if (!$cash_principal) return 0;
        $all_cash = $cash_principal->beginning_balance ?? 0;
    ...
    public function available()
    {
        ...
        foreach ($payment_methods as $method) {
            ...
            if ($methodName !== 'Transferencia') {
                ...
                $saldos[$methodName] = ($incomes - $expenses) - $transfer;
            }
        }

        // 🔹 Procesamos las cuentas bancarias por abbreviation
        $bankSaldos = [];
