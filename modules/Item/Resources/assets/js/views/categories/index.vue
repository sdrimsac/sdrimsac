        ...
        <div class="card-body">
          <data-table :resource="resource" @clickReport="clickReport" class="table-striped">
            <tr slot="heading" class="bg-primary">
              ...
              <th class="text-white" v-if="configuration.pos_drag_category">Caja arrastre</th>
            ...
            <tr slot-scope="{ index, row }">
              ...
              <td>
                <span
                  :class="{'text-white': row.active === 1, 'text-white': row.active === 0}"
                  :style="{'background-color': row.active === 1 ? 'green' : 'red', 'color': 'white', 'padding': '5px', 'border-radius': '5px'}"
                >{{ row.active === 1 ? 'Activo' : 'Inactivo' }}</span>
              </td>
              <td v-if="configuration.pos_drag_category">
                <el-checkbox @change="changeDrag(row)" v-model="row.drag"></el-checkbox>
              </td>
              <td class="text-right">
                <button
                  class="btn p-0"
...
export default {
  ...
  methods: {
    ...
    clickReport(query = null) {
      ...
      window.open(
        `/items/categories/export?column=${column || ""}&value=${value || ""}`,
        "_blank"
      );
    },
