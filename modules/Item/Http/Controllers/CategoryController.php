...
class CategoryController extends Controller
{
    ...
    public function searchItemsByCategory($category_id, Request $request)
    {
        ...
        $records = Food::where('category_food_id', $category_id);
        ...
        $records = $records->get()->take(20)->transform(function ($row) {
            return [
                'id' => $row->id,
    ...
    public function recordsDrag()
    {
        $categories = CategoryItem::where('pos_drag', true)->get()->transform(function ($row) {
            $items = Food::where('category_food_id', $row->id)->get()->take(20)->transform(function ($row) {
                return [
                    'id' => $row->id,
                    'name' => $row->description,
                ];
    ...
    public function update_category($category_id, $item_id)
    {
        ...
        $food->category_food_id = $category_id;
        $food->save();
    ...
    public function changeDrag($id)
    {
        $category = CategoryItem::findOrFail($id);
        $category->pos_drag = !$category->pos_drag;
        $category->save();
        return [
            'success' => true,
