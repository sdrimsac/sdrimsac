                                ...
                                <tbody>
                                    <tr v-for="(row, index) in form.payments" :key="index"> 
                                        ...
                                        <td class="series-table-actions text-center"> 
                                            <button  type="button" class="btn waves-effect waves-light btn-sm btn-danger" :disabled="index==0" @click.prevent="clickCancel(index)">
          ...
          methods: {
            ...
            async submit() {
              ...
              if(validate.acum_total > parseFloat(this.form.total) || validate.error_by_item > 0) {
                  return this.$toast.error('Los montos ingresados superan al monto a pagar o son incorrectos');
              }

              this.loading_submit = true;
              // await this.changePaymentMethodType(false);
              let number_full_purchase=""
              if(form.document_type_id=="01"){
                  number_full_purchase=this.form.series+"-"+this.form.number
              }else{
                number_full_purchase=this.form.series_guia+"-"+this.form.number_guia
              }
              await this.$http
                ...
                .then(response => {
                  if (response.data.success) {
                    ...
                    this.$message({
                      showClose: true,
                      message: `Compra registrada : ${number_full_purchase}`,
                      duration: 2 * 3000,
                      type: "success"
                    });
