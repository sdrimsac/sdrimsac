...
class EtiquetasController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        try {
            ...
            // Use first barcode for template generation
            $barcode = $barcodes[0];
    ...
    public function generate(Request $request)
    {
        ...
        try {
            ...
            // Generate one label for each unique barcode
            foreach ($barcodes as $index => $currentBarcode) {
                if ($index > 0) {
                    $pdf->AddPage();
                }

                // Generate only one label per barcode
                $currentStock = 1; // Set stock to 1 for each barcode
                $html = view('etiquetas::' . $template, compact(
                    'type',
                    'record',
                    'company',
