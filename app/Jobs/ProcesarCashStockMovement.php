...
class ProcesarCashStockMovement implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $row;
    protected $sale_note_item;
    protected $cash;

    public function __construct($row, $sale_note_item, $cash)
    {
        $this->row = $row;
        $this->sale_note_item = $sale_note_item;
        $this->cash = $cash;
    }
    ...
    public function handle()
    {
        try {
            $row = $this->row;
            $sale_note_item = $this->sale_note_item;

            Log::info('Iniciando ProcesarCashStockMovement para item_id: paso por aqui ');
            $cash = $this->cash;
            ...
            Log::info('ProcesarCashStockMovement debug inicio', [
                ...
                'sale_note_item_item_id' => $sale_note_item->item_id ?? null,
            ...
            $item_id = $sale_note_item->item_id ?? ($row['item_id'] ?? null);
            $quantity = $row['quantity'] ?? ($sale_note_item->quantity ?? 0);
