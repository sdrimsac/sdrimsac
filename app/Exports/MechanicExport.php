...
class MechanicExport implements FromCollection, WithHeadings, ShouldAutoSize, WithChunkReading, WithCustomStartCell, WithStyles
{
    protected $records;
    protected $company;
    ...
    public function collection()
    {
        ...
        foreach ($this->records as $row) {
            foreach ($row->historialServiceDetails as $detail) {
                $data->push([
                    'personal' => optional($row->personal)->name,
                    'vehiculo_id' => $row->vehiculo_id,
                    'placa' => $row->vehiculo->placa ?? 'No definido',
                    'modelo' => $row->vehiculo->modelo ?? 'No definido',
                    'serie' => $row->vehiculo->serie ?? 'No definido',
                    'customer' => optional($row->vehiculo->customer)->name,
                    'tipo_vehiculo' => optional($row->vehiculo->tipo_vehiculo)->description,
                    'servicio' => $detail->serviceDetail->name ?? 'No definido',
                    'precio_unitario' => $detail->serviceDetail->price_unit ?? '0.00',
                ]);
            }
        }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('A1', 'Reporte de Mecánicos');
