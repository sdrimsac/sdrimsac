    ...
    public function generarResumenAsistencias(Request $request)
    {
        ...
        foreach ($empleados as $empleado) {
            ...
            while ($i < $count) {
                ...
                // Manejar turnos nocturnos correctamente
                $startHour = $startDt->hour;
                $endHour = $endDt->hour;
                
                // Inicializar las variables de entrada y salida reales
                $actualEntrance = $startDt;
                $actualExit = $endDt;
                
                // Si el start es de madrugada (00:00-08:59) y el end es de noche (19:00-23:59)
                // entonces están intercambiados - el "end" es realmente la entrada y el "start" es la salida
                if ($startHour >= 0 && $startHour <= 8 && $endHour >= 19 && $endHour <= 23) {
                    $actualEntrance = $endDt;  // La marca de noche es la entrada
                    $actualExit = $startDt->copy()->addDay();  // La marca de madrugada es la salida del día siguiente
                } elseif ($endDt->lt($startDt)) {
                    // Caso normal donde la salida es al día siguiente
                    $actualExit = $endDt->copy()->addDay();
                }

                $dateKey = $actualEntrance->toDateString();
                if (!isset($pairsByDate[$dateKey])) {
                    $pairsByDate[$dateKey] = ['minutes' => 0, 'pairs' => []];
                }
                
                $minutes = $actualEntrance->diffInMinutes($actualExit);
                $pairsByDate[$dateKey]['minutes'] += $minutes;
                $pairsByDate[$dateKey]['pairs'][] = [
                    'entrance' => $actualEntrance->format('H:i'),
                    'exit' => $actualExit->format('H:i'),
                    'minutes' => $minutes,
                    'exit_date' => $actualExit->toDateString(),
                ];
            }
