...
use App\Http\Resources\Tenant\DetractionPaymentCollection;
use App\Models\Tenant\DetractionPayment;
use App\Models\Tenant\Receipt;
use Illuminate\Support\Str;

class DetractionPaymentController extends Controller
{
    ...
    {
        $records = DetractionPayment::where('document_id', $document_id)->get();

        return new DetractionPaymentCollection($records);
    }
    ...
    public function document($document_id)
    {
        ...
        $total_paid = collect($document->detraction_payments)->sum('payment');
    ...
    function create_receipt($request, $cash_id, $document_save, $record)
    {
        ...
        $receipt->detraction_payment_id = $record->id;
    ...
    public function store(DocumentPaymentRequest $request)
    {
        ...
        DB::connection('tenant')->transaction(function () use ($id, $request) {
            ...
            if ($request->payment_method_type_id == "01" || $request->payment_method_type_id == "04" || $request->payment_method_type_id == "11") {
                $record = DetractionPayment::firstOrNew(['id' => $id]);
    ...
    {
        $item = DetractionPayment::findOrFail($id);
        Box::where('detraction_payment_id', $id)->delete();
    ...
    public function initialize_balance()
    {
        ...
        DB::connection('tenant')->transaction(function () {
            ...
            foreach ($documents as $document) {
                ...
                $total_payments = $document->detraction_payments->sum('payment');
    ...
    public function  report($start, $end)
    {
        ...
        $documents = DetractionPayment::whereBetween('date_of_payment', [$start, $end])->get();
