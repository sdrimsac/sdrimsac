...
class BoxController extends Controller
{
    ...
    public function reports_resumen_type(Request $request)
    {
        ...
        // $period = $request['period'];
        // //$cash=Cash::where('user_id',$request['user_id'])->get()->last();
        // $date_start = $request['date_start'];
        ...
        // $data1 = Box::where('type', '1')->where('method', 'Efectivo')->where('close', $date_close)->OrderBy('date', 'asc');

        // $data2 = Box::where('type', '2')->where('close', $date_close)->OrderBy('date', 'asc');
        // $data3 = Box::where('type', '1')->where('method', 'Transferencia')->where('close', $date_close)->OrderBy('date', 'asc');
        // $data4 = Box::where('type', '1')->where('method', 'Deposito Bancario')->where('close', $date_close)->OrderBy('date', 'asc');
        // $data5 = Box::where('type', '1')->where('method', 'Tarjeta')->where('close', $date_close)->OrderBy('date', 'asc');
        // $row_ingresos = $data1->get();
        ...
        // if ($row_tarjetas != null) {
        //     $tarjetas = $row_tarjetas->tarjetas;
        // } else {
        //     $tarjetas = "0.00";
        // }
        ...
        // $company = Company::first();
        // $establishment = ($request->establishment_id) ? Establishment::findOrFail($request->establishment_id) : auth()->user()->establishment;
        // //$request->request->add(['date_start' => $date_start]);
        // $items=(new DashboardSalePurchase())->data($request->all());
        // //return $items;
        // $saldo = 0;
        // if ($request['type'] == "pdf") {
        //     $pdf = PDF::loadView('report::boxes.report_resumen_pdf', compact("type_box", "data_ingresos", "data_egresos", "establishment", "date_start",'items', "date_end", "company", "fecha_apertura", "fecha_cierre", "transferencias", "depositos", "tarjetas", "user"))->setPaper('a4', 'landscape');
        //     return $pdf->stream('Reporte_Arqueo_Caja_' . date('YmdHis') . '.pdf');
        // } else if ($request['type'] == "excel") {
