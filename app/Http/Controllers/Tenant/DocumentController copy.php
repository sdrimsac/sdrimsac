...
');
    ...
    public function store(DocumentRequest $request)
    {
        ...
        foreach ($request->items as $row_item) {
            $item = Item::find($row_item['item_id']);
            if ($item && $item->has_warranty) {
            $start_date = Carbon::parse($request->input('date_of_issue'));
            $end_date = $start_date->copy()->addMonths($item->month_day);
            ItemWarranty::create([
                'document_item_id' => $row_item['id'],
