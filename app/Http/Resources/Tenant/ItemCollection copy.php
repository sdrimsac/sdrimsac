...
class ItemCollection extends ResourceCollection
{
    ...
    public function toArray($request)
    {
        ...
        // Preload item warehouse data to avoid queries in the loop
        ...
        // Get register movements for all items at once
        ...
        return $this->collection->transform(function ($row, $key) use (
            ...
            $item_unit_types = !$row->item_unit_types ? [] : $row->item_unit_types->map(function ($unit_type) {
                return [
            ...
            $item_codes = !$row->item_codes ? [] : $row->item_codes->map(function ($code) {
                return [
            ...
            return [
                ...
                'warehouses' => $row->warehouses->map(function ($warehouse) use ($decimal) {
                    return [
                        'id' => $warehouse->id,
                ...
                'warehouse_prices' => $row->item_warehouse_prices->map(function ($price) use ($decimal) {
                    return [
                        "id" => $price->id,
