...
class WarrantyCollection extends ResourceCollection
{
    ...
    public function toArray($request)
    {
        ...
        return $this->collection->transform(function ($row) {
            ...
            $serie_vendida = $this->extraerSeriesVendidas($row->lot);
    ...
    private function extraerSeriesVendidas($lot)
    {
        $series = [];

        if ($lot && isset($lot->series)) {
            $decoded = json_decode($lot->series, true);

            if (is_array($decoded) && isset($decoded['lots'])) {
                foreach ($decoded['lots'] as $lotItem) {
                    if (isset($lotItem['series'])) {
                        $series[] = $lotItem['series'];
                    }
                }
            }
        }
