...
class DocumentLiteCollection extends ResourceCollection
{
    ...
    public function toArray($request)
    {
        ...
        return $this->collection->transform(function ($row, $key) use ($configuration) {
            ...
            // Sum of all payments (including extorned) but we will use non-extorned when computing remain/paid
            ...
            // Use payments that are NOT extorned (extorned = 0) to calculate how much has been actually paid
            // The Document model provides a paymentsNotExtorned relation and a helper totalPaymentsNotExtorned()
            // Fallback: if helper not available, sum the relation directly.
            // initialize
            $paid_amount = 0;
            try {
                $paid_amount = method_exists($row, 'totalPaymentsNotExtorned') ? $row->totalPaymentsNotExtorned() : $row->paymentsNotExtorned->sum('payment');
            } catch (\Exception $e) {
                // In case relation/helper is missing for some rows, fallback to summing payments with extorned = 0
                $paid_amount = DocumentPayment::where('document_id', $row->id)->where('extorned', 0)->sum('payment');
            }

            $remain = round($row->total - $paid_amount, 2);

            $paid = $paid_amount >= $row->total;
