...
class Facturalo
{
    ...
    public function save($inputs)
    {
        if (key_exists('id', $inputs) && $inputs['id'] != null) {
            $document_delete = Document::findOrFail($inputs['id']);
            $document_delete->delete();
            $document_delete->items()->delete();
        }
    ...
    public function createPdf($document = null, $type = null, $format = null, $college = false)
    {
        ...
        $html = $template->pdf($base_pdf_template, $this->type, $this->company, $this->document, $format_pdf, $boxes);
    ...
    private function savePayments($document, $payments)
    {
        ...
        foreach ($payments as $row) {
            ...
            //considerar la creacion de una caja general cuando recien se crea el cliente
