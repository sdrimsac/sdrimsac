...
<table class="full-width">
    <tr>
        ...
        <td width="" class="pt-3 "><p class="desc-9">{{ $document->date_of_issue }} {{substr($document->time_of_issue, 0, -3)}}</p></td>
    </tr>
...
<table class="full-width">
    ...
    @if($payments->count())
        ...
        @foreach($payments as $row)
            <tr>
                <td class="desc">&#8226; {{$row->payment_method_type->description}}: {{ $row->reference }} {{ $document->currency_type->symbol }} {{ $row->payment }}</td>
            </tr>
        @endforeach
    @endif
    <tr>
