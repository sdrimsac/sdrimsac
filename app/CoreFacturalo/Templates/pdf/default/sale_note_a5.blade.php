    ...
    <tr>
        @if($company->logo)
            <td width="40%"  height="20px">
                <div >
                    <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->name}}" class="company_logo" style="max-width: 180px;">
                </div>
            </td>
        @else
            <td width="40%"  height="20px">
...
<table class="full-width">
    ...
    <tr>
        <td height="20px" style="font-size:12px;"><b>{{ $customer->identity_document_type->description }}</b></td>
        <td  height="20px">:</td>
    ...
    @endif
    <tr>
        <td   height="18px" style="font-size:12px;padding:0px;">OBSERVACION:</td>
        <td>:</td>
        <td colspan="4" style="font-size:12px;padding:0px;">{{$document->additional_information}}</td>
    </tr>
</table>
</div>  
...
@endforeach
  ...
  @if($document->total_discount > 0)
    ...
    <tr>
        <td colspan="5" class="text-right font-bold">TOTAL VENTA: {{ $document->currency_type->symbol }}</td>
        <td class="text-right font-bold">{{ number_format($document->total, 2) }}</td>
    </tr>
    <tr>
        <td colspan="5" class="text-right font-bold border_detalles">REDONDEO: {{ $document->currency_type->symbol }}</td>
        <td class="text-right font-bold border_detalles">{{ number_format($document->total_rounded, 2) }}</td>
    </tr>
    <tr>
        <td colspan="5" class="text-right font-bold border_detalles">TOTAL A PAGAR: {{ $document->currency_type->symbol }}</td>
...
<tr>
<td align="center" width="45%" style="border-top:1px solid #000;font-weight:bold">Recibi Conforme<br>Cliente</td>
<td></td>
