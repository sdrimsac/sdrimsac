...
<table class="full-width" style="margin-left:15px;margin-right:15px;">
    <tr>
        <td class="text-center"><h4>{{ $company->eslogan }}</h4></td>
    </tr>
    <tr>
...
<table class="full-width mt-10 mb-10" style="margin-left:15px;margin-right:25px;">
    <thead class="">
    <tr>
    ...
    @endif
        ...
        <tr>
            <td align="right">EFECTIVO: {{ $document->currency_type->symbol }} {{number_format(Session::get('enter_amount'),2) }}</td>
        </tr>
        <tr>
        <td align="right">VUELTO: {{ $document->currency_type->symbol }} {{number_format(Session::get('difference'),2) }}</td>
        </tr>
    @endif
        {{-- @if($balance < 0)
           <tr>
...
<table class="full-width" style="margin-left:15px;margin-right:15px;">
    ...
    <?php
        ...
        $turned=App\Models\Tenant\Turned::where('number',$company->number)->first();
    ?>
    @if($turned!=null)
    @if($turned->enter_amount>0)
    <tr>
        <td align="right">EFECTIVO: {{ $document->currency_type->symbol }} {{ number_format($turned->enter_amount,2)}}
        </td>
      </tr>
    <tr>
      <td align="right">VUELTO: {{ $document->currency_type->symbol }} {{number_format($turned->difference,2)}}</td>
    </tr>
    @endif
    @endif
    <tr>
