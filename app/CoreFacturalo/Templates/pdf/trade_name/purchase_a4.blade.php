...
<body>
    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr class="bg-grey">
                ...
                <th class="border-top-bottom text-center py-2" width="16%">ALMACEN</th>
                        ...
                        @if (isset($row->item->color_size))
                            @foreach ($row->item->color_size as $color_size)
                                <br />{!! '<strong>Color: </strong>' . $color_size->color !!} {!! ' <strong>Talla:</strong> ' . $color_size->size !!} <strong>-
                                    Cant:</strong> {{ $color_size->stock }}
                            @endforeach
                        @endif
                        @if (isset($row->item->lots_group) && count($row->item->lots_group) > 0)
                            <br>
                            Lotes:
                            @foreach ($row->item->lots_group as $idx => $lots_group)
                                <small>
                                    {!! $lots_group->code !!}
                                    @if ($idx + 1 < count($row->item->lots_group))
                                        -
                                    @endif
                                </small>
                            @endforeach
                        @endif
                        @if (isset($row->item->lots) && count($row->item->lots) > 0)
                            <br>
                            Series:
                            @foreach ($row->item->lots as $idx => $lot)
                                <small>
                                    {!! $lot->series !!}
                                    @if ($idx + 1 < count($row->item->lots))
                                        -
                                    @endif
                                </small>
                            @endforeach
   ...
   {{--  <div>
    ...
    @if ($document->fee && $document->fee->count())
