...
<body>
    ...
    @if ($company->logo)
        <div class="text-center company_logo_box_sm pt-5 desc-9">
            <img src="data:{{ mime_content_type(public_path("storage/uploads/logos/{$company->logo}")) }};base64, {{ base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}"))) }}"
                alt="{{ $company->trade_name }}" class="company_logo_sm content">
        </div>
        {{-- @else --}}
    ...
    <table class="full-width">
        ...
        @isset($establishment->trade_address)
            <tr>
                <td class="text-center ">
                    {{ $establishment->trade_address !== '-' ? '  ' . $establishment->trade_address : '' }}</td>
            </tr>
        @endisset
        <tr>
    ...
    <table class="full-width">
        <tr>
            <td width="" class="pt-3">
                <p class="desc-9">F. Emisión:</p>
            </td>
            <td width="" class="pt-3 ">
                <p class="desc-9">{{ $document->date_of_issue }}</p>
            </td>
        </tr>
        ...
        @isset($invoice->date_of_due)
            <tr>
                <td>
                    <p class="desc-9">F. Vencimiento:</p>
                </td>
                <td>
                    <p class="desc-9">{{ $invoice->date_of_due }}</p>
                </td>
            </tr>
        @endisset
        ...
        <tr>
            ...
            <td>
                <p class="desc-9">{{ $customer->name }}</p>
            </td>
        </tr>
        <tr>
            <td>
                <p class="desc-9">{{ $customer->identity_document_type->description }}:</p>
            </td>
            <td>
                <p class="desc-9">{{ $customer->number }}</p>
            </td>
        </tr>
        @if ($customer->address !== '')
            <tr>
                <td class="align-top">
                    <p class="desc-9">Dirección:</p>
                </td>
                <td>
                    <p class="desc-9">
                        {{ $customer->address }}
        ...
        @if ($document->purchase_order)
            <tr>
                <td>
                    <p class="desc-9">Orden de Compra:</p>
                </td>
                <td>
                    <p class="desc-9">{{ $document->purchase_order }}</p>
                </td>
            </tr>
        @endif
        @isset($document->quotation->delivery_date)
            <tr>
                <td>
                    <p class="desc-9">F. Entrega</p>
                </td>
                <td>
                    <p class="desc-9">{{ $document->quotation->delivery_date }}</p>
                </td>
            </tr>
        @endisset
        @isset($document->quotation->sale_opportunity)
            <tr>
                <td>
                    <p class="desc-9">O. Venta</p>
                </td>
                <td>
                    <p class="desc-9">{{ $document->quotation->sale_opportunity->number_full }}</p>
                </td>
            </tr>
        @endisset
    </table>
    ...
    <table class="full-width mt-10 mb-10">
        ...
        <tbody>
            ...
            @if ($document->total_exportation > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc-9">OP. EXPORTACIÓN:
            ...
            @if ($document->total_free > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc-9">OP. GRATUITAS:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_unaffected > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc-9">OP. INAFECTAS:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_exonerated > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc-9">OP. EXONERADAS:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_taxed > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc-9">OP. GRAVADAS:
                        {{ $document->currency_type->symbol }}</td>
            ...
            @if ($document->total_plastic_bag_taxes > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">ICBPER: {{ $document->currency_type->symbol }}
                    </td>
    ...
    <table class="full-width">
        ...
        <tr>
            @if ($row->code == '1000')
                <td class="desc-9 pt-3">Son: <span class="font-bold">{{ $row->value }}
                        {{ $document->currency_type->description }}</span></td>
                @if (count((array) $document->legends) > 1)
        <tr>
            <td class="desc-9 pt-3"><span class="font-bold">Leyendas</span></td>
        </tr>
        @endif
    @else
            ...
            <td class="desc-9 pt-3">
                ...
                @if (in_array($document->document_type->id, ['01', '03']))
                    @foreach ($accounts as $account)
                        ...
                        {{ $account->currency_type->description }} {{ $account->number }}
                    @endforeach
                @endif
        ...
        <tr>
            <td class="text-center pt-3 company_logo_box_sm"><img class="content"
        ...
        @if ($footer_text)
            <tr>
                <td class="text-center desc pt-5">$footer_text</td>
            </tr>
        @endif
        <tr>
