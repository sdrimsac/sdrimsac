...
@if($company->logo)
    <div class="text-center company_logo_box_sm pt-5 desc-9">
        <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->trade_name}}" class="company_logo_sm content">
    </div>
{{--@else--}}
...
<table class="full-width">
    ...
    <tr>
        <td class="text-center"> <h6>{{ $company->eslogan }}</h6></td>
    </tr>
    <tr>
...
<table class="full-width">
    <tr>
        ...
        <td width="" class="pt-3 "><p class="desc-9">{{ $document->date_of_issue }}</p></td>
    </tr>

    @isset($invoice->date_of_due)
    <tr>
        <td><p class="desc-9">F. Vencimiento:</p></td>
        <td><p class="desc-9">{{ $invoice->date_of_due }}</p></td>
    </tr>
    @endisset
