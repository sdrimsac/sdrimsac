...
<table class="full-width">
    <tr>
        @if($company->logo)
            <td width="10%">
                <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->trade_name}}" alt="{{ $company->trade_name }}" class="company_logo" style="max-width: 200px">
            </td>
        @endif
...
<table class="full-width mt-5">
    <tr>
        <td width="25%">Señor(es): jajaj</td>
        <td width="45%">{{ $customer->name }}</td>
        <td width="15%">Fecha de emisión:</td>
        <td width="15%">{{ \Carbon\Carbon::parse($document->date_of_issue)->format('d-m-Y') }}</td>
    </tr>
    <tr>
    ...
    <tr>
        <td>Régimen de percepción:</td>
        <td colspan="3">{{ $document->perception_type->description }}</td>
    </tr>
</table>
<table class="full-width mt-10 mb-10">
    ...
    <tr class="bg-grey">
        ...
        <th class="border-top-bottom text-center desc">Importe<br/>Percibido</th>
    ...
    @foreach($document->documents as $row)
        <tr>
            ...
            <td class="text-center">{{ \Carbon\Carbon::parse($row->date_of_issue)->format('d-m-Y') }}</td>
            <td class="text-center">{{ $row->currency_type_id }}</td>
            <td class="text-right">{{ $row->total_document }}</td>
            <td class="text-center">{{ $document->perception_type->percentage }}</td>
