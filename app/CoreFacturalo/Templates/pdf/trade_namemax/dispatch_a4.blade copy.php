@php
    ...
    $document_type_driver = App\Models\Catalogs\IdentityDocumentType::findOrFail($document->driver->identity_document_type_id);
@endphp
<html>
<head>
...
<table class="full-width">
    <tr>
        @if($company->logo)
            <td width="10%">
                <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->trade_name}}" alt="{{ $company->trade_name }}"  class="company_logo" style="max-width: 300px">
            </td>
        @else
            <td width="10%">
...
<table class="full-width border-box mt-10 mb-10">
    ...
    <tr>
        <td>Motivo Traslado: {{ ($document->transfer_reason_type->id!="13") ? $document->transfer_reason_type->description : "Consignación" }} </td>
