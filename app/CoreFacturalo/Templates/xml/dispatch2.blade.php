...
<DespatchAdvice xmlns="urn:oasis:names:specification:ubl:schema:xsd:DespatchAdvice-2"
    ...
    @if ($document->related)
        <cac:AdditionalDocumentReference>
            ...
            <cbc:DocumentTypeCode listAgencyName="PE:SUNAT" listName="SUNAT:Identificador de documento relacionado"
                listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo21">{{ $document->related->document_type_id }}
    ...
    <cac:DeliveryCustomerParty>
        <cbc:CustomerAssignedAccountID schemeID="{{ $document->customer->identity_document_type_id }}">
            {{ $document->customer->number }}</cbc:CustomerAssignedAccountID>
        <cac:Party>
            <cac:PartyLegalEntity>
                <cbc:RegistrationName>
                    <![CDATA[{{ $document->customer->name }}]]>
                </cbc:RegistrationName>
            </cac:PartyLegalEntity>
        </cac:Party>
    </cac:DeliveryCustomerParty>
    ...
    <cac:Shipment>
        ...
        @endif
        <cbc:GrossWeightMeasure unitCode="{{ $document->unit_type_id }}">{{ $document->total_weight }}
        </cbc:GrossWeightMeasure>
        @if ($document->packages_number)
            <cbc:TotalTransportHandlingUnitQuantity>{{ $document->packages_number }}
            </cbc:TotalTransportHandlingUnitQuantity>
        @endif
        <cbc:SplitConsignmentIndicator>{{ $document->transshipment_indicator ? 'true' : 'false' }}
            ...
            @if ($document->dispatcher)
                ...
                <cac:CarrierParty>
                    <cac:PartyIdentification>
                        <cbc:ID schemeID="{{ $dispatcher->identity_document_type_id }}">{{ $dispatcher->number }}
                        </cbc:ID>
                    </cac:PartyIdentification>
                    <cac:PartyName>
                        <cbc:Name>
                            <![CDATA[{{ $dispatcher->name }}]]>
                        </cbc:Name>
                    </cac:PartyName>
                </cac:CarrierParty>
            @endif
    ...
    @foreach ($document->items as $row)
        <cac:DespatchLine>
            ...
            <cbc:DeliveredQuantity unitCode="{{ $row->item->unit_type_id }}">{{ $row->quantity }}
