...
class DocumentInput
{
    ...
    public static function set($inputs)
    {
        ...
        if (array_key_exists('data_json', $inputs)) {

            if (array_key_exists('observacion', $inputs['data_json'])) {
                $observation = $inputs['data_json']['observacion'];
            } else {
                $observation = null;
            }
        } else {

            $observation = Functions::valueKeyInArray($inputs, 'observation', null);
        }
        $printerOn = false;
        if (array_key_exists('printerOn', $inputs)) {
            $printerOn = $inputs['printerOn'];
        } else {
            $printerOn = false;
        }
        $total_payment = 0;
        if (array_key_exists('total_payment', $inputs)) {
            $total_payment = $inputs['total_payment'];
        } else {
        ...
        return [
            ...
            'met hod_pay' => Functions::valueKeyInArray($inputs, 'method_pay', 'Efectivo'),
            'printerOn' => $printerOn,
            ...
            'total_taxes' => Functions::valueKeyInArray($inputs, 'total_taxed', 0),
            'sale_notes_relateds' => Functions::valueKeyInArray($inputs, 'sale_notes_relateds', []),
            ...
            'total' => Functions::valueKeyInArray($inputs, 'total', 0),
            'total_rounded' => Functions::valueKeyInArray($inputs, 'total_rounded', Functions::valueKeyInArray($inputs, 'total', 0)),
            ...
            foreach ($inputs['items'] as $row) {
                ...
                $items[] = [
                    ...
                    'item' => [
                        ...
                        'unit_type_id' => (key_exists('item', $row)) ? Functions::valueKeyInArray($row['item'], 'unit_type_id', $item->unit_type_id) : $item->unit_type_id,
                        ...
                        'is_set' => $item->is_set,
                        'promotions_items' => $item->promotions_items,
                        ...
                        'item_codes_scanned' => (isset($row['item']['item_codes_scanned'])) ? $row['item']['item_codes_scanned'] : [],
    ...
    private static function discounts($inputs)
    {
        if (array_key_exists('discounts', $inputs)) {
            if ($inputs['discounts']) {
                if (is_array($inputs['discounts']) && !is_array($inputs['discounts'][array_key_first($inputs['discounts'])])) {
                    $inputs['discounts'] = [$inputs['discounts']];
                }
    ...
    private static function detraction($inputs)
    {
        if (array_key_exists('detraction', $inputs)) {
            if ($inputs['detraction']) {
                ...
                if (isset($detraction['detraction_type_id'])) {
