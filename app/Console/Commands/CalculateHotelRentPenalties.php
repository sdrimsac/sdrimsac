...

{
    ...
    {
        $pendingRents = HotelRentItem::where('payment_status', 'pending')
    ...
    private function calculatePenalty($rentId)
    {
        ...
        $state_type_id = $table->state_type_id;
        $hotel_rent_penalties_settings = HotelRentPenaltySetting::where('state_type_id', $state_type_id)->first();
        if(!$hotel_rent_penalties_settings){
            $hotel_rent_penalties_settings = HotelRentPenaltySetting::whereNull('state_type_id')->first();
        }
        if(!$hotel_rent_penalties_settings){
            return;
        }
