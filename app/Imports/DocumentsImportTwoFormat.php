    ...
    public function collection(Collection $rows)
    {
            ...
            foreach ($rows as $row)
            {
               ...
               // $serienumero = explode('-', $nrodocumento);
                ...
                //fecha de documento
                $create_date = $row[3];
               // dd($serie,$number,$correlativo,$document_type,$document_type,$document_type_operation,$row[3]);
                ...
                //moneda
                $currency = ($row[6] == 'S') ? 'PEN' : 'Registre con los administradores nueva moneda' ;

                //cliente
                $co_number = rtrim($row[4]);
 
                    if ($co_number > 0 || $co_number!=null) {
               ...
               // if ($row[17] != null) {
                    ...
                    //valores
                    $cantidad_s = $row[13];
                    $precio_unitario_s = $row[14];
                    $subtotal_s = $row[15];
                    $total_impuesto_s = $row[16];
                    $new_item = [
                            "codigo_interno" =>'',
                            "descripcion" => rtrim($row[11]),
                            "codigo_producto_sunat" => "",
