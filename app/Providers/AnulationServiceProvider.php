            ...
            if ($document['document_type_id'] == '01' || $document['document_type_id'] == '03') {
                if ($document['state_type_id'] == 11) {
                    ...
                    if (!$has_sale_note) {
                        foreach ($document['items'] as $detail) {
                            $item = Item::find($detail['item_id']);
                            $item_set = $item->sets()->first();

                            if ($item_set) {
                                $individual_items = $item->sets()
                                    ->get()
                                    ->map(function($set) {
                                        return [
                                            'item_id' => $set->individual_item_id,
                                            'quantity' => $set->quantity
                                        ];
                                    });
                                foreach ($individual_items as $individual_item) {
                                    $total_quantity = $detail['quantity'] * $individual_item['quantity'];
                                    $item = Item::find($individual_item['item_id']);
                                    $item->stock = $item->stock + $total_quantity;
                                    $item->save();
                                    
                                    $this->updateStock($individual_item['item_id'], $total_quantity, false);
                                    $this->saveKardex('sale', $individual_item['item_id'], $document['id'], -$total_quantity, 'document');
                                }
                            } else {
                                $item->stock = $item->stock + $detail['quantity'];
                                $item->save();
