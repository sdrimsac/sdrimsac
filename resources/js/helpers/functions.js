function calculateRowItem(
    ...
    let record_id = row_old.record_id
        ? row_old.record_id
        : row_old.id
        ? row_old.id
        : null;
        ...
        row.discounts.forEach((discount, index) => {
            ...
            if (discount.is_amount) {
                if (discount.discount_type.base) {
                    ...
                    discount.percentage = _.round(
                        100 *
                            (parseFloat(discount.amount) /
                                parseFloat(discount.base)),
                        5
                    );
    ...
    if (row_old.has_plastic_bag_taxes) {
        total_plastic_bag_taxes = _.round(
    ...
    let total_taxes =
        total_igv + total_isc + total_other_taxes + total_plastic_bag_taxes;
    ...
    if (row.discounts.length > 0) {
        ...
        row.discounts.forEach(discount => {
            sum_discount_no_base +=
                discount.discount_type_id == "01" ? discount.amount : 0;
            sum_discount_base +=
                discount.discount_type_id == "00" ? discount.amount : 0;
        });
    ...
    console.log("la afectacion igv es: ", row.affectation_igv_type);
...
function getAmountFromInputDiscount(discount) {
    ...
    if (
        discount.use_input_amount !== undefined &&
...
const filterWords = (input, items) => {
    ...
    return items.filter(item => {
        ...
        return !search_value.split(" ").some(p => !text_filter.includes(p));
    });
};
