    ...
    >
        <div class="">
            <div class="row">
                ...
                <div class="col-md-4">
                    <h5 class="fw-bold">
                        Total Acumulado S/ {{ totalAcumulado }}
                    </h5>
                    <br />
                    <h5 class="fw-bold">
                        Total Cantidad Vendida: {{ display_quantity_sold }}
                    </h5>
                </div>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        total_quantity_sold() {
            ...
            return this.combinedItems.reduce(
                (sum, item) => sum + Number(item.quantity || 0),
                0
            );
        },
        display_quantity_sold() {
            // Prioriza el valor entregado por el backend si viene (>0), sino calcula local
            if (
                this.server_total_quantity_sold &&
                Number(this.server_total_quantity_sold) > 0
            ) {
                return this.server_total_quantity_sold;
            }
    ...
    methods: {
      ...
      this.sold_items = [...this.sold_items, ...this.salesItems];
        ...
        clickDownload() {
            ...
            window.open(
                `/${this.resource}/list-product?${queryString}`,
                "_blank"
            );
        },
        productSeller() {
                ...
                .then(response => {
                    const first =
                        response.data.data && response.data.data[0]
                            ...
                            : {};
                    this.sold_items = first.sold_items || [];
                    this.salesItems = first.salesItems || [];
                    this.server_total_quantity_sold =
                        first.total_quantity_sold || 0;
                    this.combineItems();
