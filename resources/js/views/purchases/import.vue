                        ...
                        >
                            ...
                            >
                                <el-button slot="trigger" type="primary"
                                    >Seleccione un archivo (xml)</el-button
                                >
                            </el-upload>
                            <small
                ...
                <div>
                    <div v-if="previewData">
                        ...
                        <p>
                            ...
                            {{ previewData.date_of_issue }}
                        </p>
                        <p><strong>Total:</strong> {{ previewData.total }}</p>
    ...
    methods: {
        getText(value) {
            ...
            if (typeof value === "object" && value !== null) {
                return value._text?.trim() || value.__cdata?.trim() || "";
            }
            return "";
        },
        handleChange(file) {
        ...
        async parseXml(source) {
            ...
            this.formXmlJson = convert.xml2js(source, {
                compact: true,
                spaces: 4
            });
            await this.setSuplierForm();
            ...
            try {
                ...
                const supplierData = {
                    ...
                    district:
                        Invoice["cac:AccountingSupplierParty"]["cac:Party"][
                            "cac:PartyLegalEntity"
                        ]["cac:RegistrationAddress"]["cbc:District"]["_text"],
                    province:
                        Invoice["cac:AccountingSupplierParty"]["cac:Party"][
                            "cac:PartyLegalEntity"
                        ]["cac:RegistrationAddress"]["cbc:CityName"]["_text"],
                    department:
                        Invoice["cac:AccountingSupplierParty"]["cac:Party"][
                            "cac:PartyLegalEntity"
                    ...
                    ubigeo:
                        Invoice["cac:AccountingSupplierParty"]["cac:Party"][
                            "cac:PartyLegalEntity"
                        ]["cac:RegistrationAddress"]["cbc:ID"]["_text"],
                    country_id:
                ...
                this.$message.error(
                    "No se pudo registrar automáticamente al proveedor."
                );
            }
        },
        ...
        async setdataForm() {
            ...
            this.form.total_taxes =
                Invoice["cac:TaxTotal"]["cbc:TaxAmount"]["_text"];
            ...
            if (Array.isArray(items)) {
                items.forEach(element => {
                    let code =
                        ...
                        ]?.["cbc:ItemClassificationCode"]?._text;

                    let formItem = self.initFormItem();

                    if (!code) {
                        const description =
                            element?.["cac:Item"]?.["cbc:Description"]?._text ||
                            "";

                        formItem.item = _.find(this.items, item =>
                            item.description
                                ?.toLowerCase()
                                .includes(description.toLowerCase())
                        );

                        if (!formItem.item) return;
                    } else {
                ...
                if (!code) {
                    const description =
                        items?.["cac:Item"]?.["cbc:Description"]?._text || "";
        ...
        xmlToJson(xml) {
            ...
            if (
                xml.hasChildNodes() &&
                xml.childNodes.length === textNodes.length
            ) {
                obj = [].slice
                    .call(xml.childNodes)
                    .reduce(function(text, node) {
