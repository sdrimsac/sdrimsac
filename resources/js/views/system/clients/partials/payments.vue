            ...
            <div class="row">
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td>{{ row.reference }}</td> -->
                                        ...
                                        >
                                        <button
                                        v-if="row.document && row.document_url"
                                        type="button"
                                        class="btn waves-effect waves-light btn-xs btn-success"
                                        @click.prevent="
                                            openDocument(row.document_url)
                                        "
                                        >
                                        {{ row.document }}
                                        </button>
                ...
                <div class="col-md-12 text-center pt-2">
                    <el-button
                        ...
                        icon="el-icon-plus"
                        @click="clickAddRow"
                        >Programar pago</el-button
                    >
                </div>
            </div>
            <client-messages
                :showDialog.sync="showDialogMessages"
                :client_payment_id="recordId"
            ></client-messages>
            <payment-form
...
export default {
    ...
    methods: {
        ...
        async getData() {
            this.initForm();
            await this.$http
                .get(`/${this.resource}/client/${this.clientId}`)
                .then(response => {
            ...
            await this.$http
                .get(`/${this.resource}/records/${this.clientId}`)
                .then(response => {
        ...
        clickSubmit(index) {
            ...
            let payment_method_type_id = this.payment_method_types.find(
                payment_method_type =>
                    payment_method_type.description === "Efectivo"
            ).id;
            let form = {
