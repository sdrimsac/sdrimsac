    ...
    <div class="container-fluid p-l-0 p-r-0">
      <div class="card mb-0">
        <div class="card-header bg-primary">
          <h4 class="my-0 text-white">
            ...
            Listado de Ingreso y Salida de Productos
          </h4>
        </div>
        ...
        <div class="card-body">
          <data-table :resource="resource" ref="dataTable">
            ...
            <tr slot-scope="{ index, row }">
              ...
              <td class="text-left">{{ parseFloat(row.quantity).toFixed(2) }}</td>
              <td class="text_left">{{ row.created_at }}</td>
              <td class="text_end">
                <span
                  :class="{'bg-success text-white rounded-pill px-2 py-1': row.type === 'input', 'bg-danger text-white rounded-pill px-2 py-1': row.type === 'output'}"
                >{{ row.type === 'input' ? 'ingreso' : 'salida' }}</span>
              </td>
              <!-- <td>
              ...
              <td>
                <button @click="clickPrint(row.id)" type="button" class="btn btn-sm btn-primary">
                  <i class="fa fa-print"></i>
                </button>
              </td>
            </tr>
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="showDialogEstablishment = false">Cancelar</el-button>
