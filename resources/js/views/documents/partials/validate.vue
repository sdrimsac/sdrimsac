        ...
        >
            <form autocomplete="off">
                ...
                >
                    <div class="row">
                        <div class="col-md-12">
                            <label class="control-label w-100">Periodo</label>
                            <el-select
                                v-model="form.period"
                                @change="changePeriod"
                            >
                        ...
                        <div class="col-lg-12 col-md-12 ">
                            ...
                            >
                                <label class="control-label w-100"
                                    >Comprobante<span class="text-danger">
                                        *</span
                                    ></label
                                >
...
export default {
    ...
    methods: {
        ...
        async initForm() {
            ...
            await this.$http
                .get(`/reports/validate-documents/data_table`)
                .then(response => {
                    this.document_types = response.data.document_types;
        ...
        async getRecords() {
            ...
            this.acumulado = 0;
            const response = await this.$http.get(
                `/reports/validate-documents/validate_masivo?${this.getQueryParameters()}`
            );
            let responses = false;
            if (response.data.success) {
                ...
                for (
                    let index = 0;
                    index < response.data.archivo_txt;
                    index++
                ) {
                    await this.$http
                        .get(
                            `/reports/validate-documents/validateDocumentstxt?numero=` +
                                index
                        )
                        .then(response => {
                ...
                if (responses == true) {
                    ...
                    const respusta = await this.$http.get(
                        `/reports/validate-documents/validatecount?${this.getQueryParameters()}`
                    );
                    this.$alert(
                        "<b>Comprobante Aceptados: " +
                            respusta.data.aceptados +
                            "<br> Anulados: " +
                            respusta.data.anulados +
                            "<br/> Registrados: " +
                            respusta.data.registrados +
                            "</b>",
                        "Validacion Comprobantes Electronicos",
                        {
