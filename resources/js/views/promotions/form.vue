                ...
                <el-tab-pane label="General" name="general">
                    <div class="form-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mt-2" v-if="form.promotion_items.length > 0">
                                <div class="table-responsive">
                                    <table class="table table-striped">
                                        <thead>
                                            <tr slot="heading" class="bg-primary">
                                                ...
                                                <th class="text-white text-right font-weight-bold">
                                                    Cantidad
                                                </th><th class="text-white text-right font-weight-bold">
                                                    Cantidad Maxima
                                                </th>
                                        ...
                                        <tbody>
                                            <tr v-for="(row,
                                                index) in form.promotion_items" :key="index">
                                                <td>{{ index + 1 }}</td>
                                                <td>
...
export default {
    ...
    methods: {
        clickRemoveItem(index) {
            this.form.promotion_items.splice(index, 1);
            this.changeIndividualItems();
        },
        addRow(row) {
            let exist = this.form.promotion_items.find(
                item => item.promotion_item_id == row.promotion_item_id
            );

            if (exist) {
                exist.quantity += row.quantity;
            } else {
                this.form.promotion_items.push(row);
            }
            // console.log(row)
            this.changeIndividualItems();
        },
        ...
        changeIndividualItems() {
            // Placeholder for compatibility with child components that call this method
            // Previously some implementations recalculated accumulated prices from
            // individual items. Keep as a no-op to avoid breaking behavior.
        },
        initForm() {
            ...
            if (this.recordId) {
                this.$http
                    ...
                    .then(response => {
                        ...
                        // Normalize item list: some APIs return `individual_items` while
                        // the component expects `promotion_items`.
                        if (!this.form.promotion_items || this.form.promotion_items.length === 0) {
                            if (this.form.individual_items && this.form.individual_items.length > 0) {
                                this.form.promotion_items = this.form.individual_items;
                            } else {
                                this.form.promotion_items = [];
                            }
                        }
        ...
        loadRecord() {
            if (this.recordId) {
                this.$http
                    ...
                    .then(response => {
                        this.form = response.data.data;
                        // Normalize item list name from backend -> frontend expected
                        if (!this.form.promotion_items || this.form.promotion_items.length === 0) {
                            if (this.form.individual_items && this.form.individual_items.length > 0) {
                                this.form.promotion_items = this.form.individual_items;
                            } else {
        ...
        submit() {
            ...
            if (this.form.promotion_items.length < 1)
