  ...
  methods: {
    ...
    create() {
      ...
      if (this.external) {
        ...
        console.log("🚀 ~ create ~ this.input_person:", this.input_person);
        if (this.input_person) {
          ...
          this.form.number = this.input_person.number
            ...
            : "";
          if (this.input_person.number && this.input_person.number != "") {
    ...
    async generateNumber() {
      this.loading = true;
      try {
        const response = await this.$http.get(`/customers/generate_number`);
