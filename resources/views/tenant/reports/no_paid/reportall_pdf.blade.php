    ...
    <header>
        <table  style="border-collapse: collapse;border-bottom:1px solid #ddd;">
             <tr>
                 ...
                 <td width="30%" height="30" align="right" valign="middle" style="font-size:13px !important;  font-family: sans-serif !important;">Reporte de Cuentas por Cobrar</td>
             </tr>
        </table>
     </header>
        <br>
                    ...
                    <table class="">
                        ...
                        <tbody>
                           ...
                           @endphp
                            ...
                            @php
                            $total_to_pay+= $value['total_to_pay'];
                            $total+= $value['total'];   
                           @endphp
                                @if($value['total_to_pay'] > 0)
                                    <tr>
                                        <td class="celda_center">{{$loop->iteration}}</td>
                                        <td class="celda_center">{{ \Carbon\Carbon::parse($value['date_of_issue'])->format('d/m/Y') }}</td>
                                        <td class="celda">{{$value['number_full']}}</td>
                                         <td class="celda">{{$value['customer_name']}}</td>
                                        <td class="celda_center">{{$value['total_to_pay']}}</td>
                                        <td class="celda_center">{{$value['total']}}</td>
                                    </tr>
                                @endif
                              
                            @endforeach
                            <tr>
                                <td class="celda_right" colspan="4" >Total </td>
                  
                                <td class="celda_center"><b>{{$total_to_pay}}</b></td>
                                <td class="celda_center"><b>{{$total}}</b></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
